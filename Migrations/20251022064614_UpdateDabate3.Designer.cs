// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using StudentActivities.src.Data;

#nullable disable

namespace StudentActivities.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20251022064614_UpdateDabate3")]
    partial class UpdateDabate3
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.21")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("StudentActivities.src.Models.AcademicClasses", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("FacultyId")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(true)
                        .HasColumnType("character varying(20)");

                    b.HasKey("Id");

                    b.HasIndex("FacultyId");

                    b.ToTable("AcademicClasses");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Clubs", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int?>("AdminsId")
                        .HasColumnType("integer");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(200)
                        .IsUnicode(true)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(true)
                        .HasColumnType("character varying(20)");

                    b.Property<int>("OrganizerId")
                        .HasColumnType("integer");

                    b.Property<int?>("StudentsId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("AdminsId");

                    b.HasIndex("OrganizerId");

                    b.HasIndex("StudentsId");

                    b.ToTable("Clubs");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Events", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int?>("AdminsId")
                        .HasColumnType("integer");

                    b.Property<int>("CurrentRegistrations")
                        .HasColumnType("integer");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<string>("Location")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)");

                    b.Property<int>("MaxCapacity")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(true)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("OrganizerId")
                        .HasColumnType("integer");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.Property<string>("Status")
                        .HasColumnType("text");

                    b.Property<int?>("StudentsId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("AdminsId");

                    b.HasIndex("OrganizerId");

                    b.HasIndex("StudentsId");

                    b.ToTable("Events");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Faculties", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(true)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.ToTable("Faculties");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Notifications", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("ClubId")
                        .HasColumnType("integer");

                    b.Property<string>("Context")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<int>("EventId")
                        .HasColumnType("integer");

                    b.Property<DateOnly>("SendDate")
                        .HasColumnType("date");

                    b.HasKey("Id");

                    b.HasIndex("ClubId");

                    b.HasIndex("EventId");

                    b.ToTable("Notifications");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Report", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("AdminId")
                        .HasColumnType("integer");

                    b.Property<int>("OrganizerId")
                        .HasColumnType("integer");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("Id");

                    b.HasIndex("AdminId");

                    b.HasIndex("OrganizerId");

                    b.ToTable("Reports");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Resgistrations", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("ClubId")
                        .HasColumnType("integer");

                    b.Property<DateOnly>("DateResgistered")
                        .HasColumnType("date");

                    b.Property<int>("EventId")
                        .HasColumnType("integer");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("StudentId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ClubId")
                        .IsUnique();

                    b.HasIndex("EventId")
                        .IsUnique();

                    b.HasIndex("StudentId");

                    b.ToTable("Resgistrations");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Semester", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(true)
                        .HasColumnType("character varying(50)");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.HasKey("Id");

                    b.ToTable("Semesters");
                });

            modelBuilder.Entity("StudentActivities.src.Models.TrainingScores", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateOnly>("DateAssigned")
                        .HasColumnType("date");

                    b.Property<int>("Score")
                        .HasColumnType("integer");

                    b.Property<int>("SemesterId")
                        .HasColumnType("integer");

                    b.Property<int>("StudentId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("SemesterId");

                    b.HasIndex("StudentId")
                        .IsUnique();

                    b.ToTable("TrainingScores");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Users", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateOnly>("Birth")
                        .HasColumnType("date");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(true)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(true)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.HasKey("Id");

                    b.ToTable("Users");

                    b.HasDiscriminator().HasValue("Users");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("StudentActivities.src.Models.Admins", b =>
                {
                    b.HasBaseType("StudentActivities.src.Models.Users");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<int?>("TrainingScoresId")
                        .HasColumnType("integer");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasIndex("TrainingScoresId");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("Users", t =>
                        {
                            t.Property("Role")
                                .HasColumnName("Admins_Role");

                            t.Property("TrainingScoresId")
                                .HasColumnName("Admins_TrainingScoresId");

                            t.Property("UserId")
                                .HasColumnName("Admins_UserId");
                        });

                    b.HasDiscriminator().HasValue("Admins");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Organizers", b =>
                {
                    b.HasBaseType("StudentActivities.src.Models.Users");

                    b.Property<int>("ClubId")
                        .HasColumnType("integer");

                    b.Property<int>("EventId")
                        .HasColumnType("integer");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<int?>("TrainingScoresId")
                        .HasColumnType("integer");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasIndex("TrainingScoresId");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("Users", t =>
                        {
                            t.Property("Role")
                                .HasColumnName("Organizers_Role");

                            t.Property("UserId")
                                .HasColumnName("Organizers_UserId");
                        });

                    b.HasDiscriminator().HasValue("Organizers");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Students", b =>
                {
                    b.HasBaseType("StudentActivities.src.Models.Users");

                    b.Property<int>("AcademicClassId")
                        .HasColumnType("integer");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasIndex("AcademicClassId");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.HasDiscriminator().HasValue("Students");
                });

            modelBuilder.Entity("StudentActivities.src.Models.AcademicClasses", b =>
                {
                    b.HasOne("StudentActivities.src.Models.Faculties", "Faculties")
                        .WithMany("AcademicClasses")
                        .HasForeignKey("FacultyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Faculties");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Clubs", b =>
                {
                    b.HasOne("StudentActivities.src.Models.Admins", null)
                        .WithMany("Clubs")
                        .HasForeignKey("AdminsId");

                    b.HasOne("StudentActivities.src.Models.Organizers", "Organizers")
                        .WithMany("Clubs")
                        .HasForeignKey("OrganizerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Students", null)
                        .WithMany("Clubs")
                        .HasForeignKey("StudentsId");

                    b.Navigation("Organizers");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Events", b =>
                {
                    b.HasOne("StudentActivities.src.Models.Admins", null)
                        .WithMany("Events")
                        .HasForeignKey("AdminsId");

                    b.HasOne("StudentActivities.src.Models.Organizers", "Organizers")
                        .WithMany("Events")
                        .HasForeignKey("OrganizerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Students", null)
                        .WithMany("Events")
                        .HasForeignKey("StudentsId");

                    b.Navigation("Organizers");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Notifications", b =>
                {
                    b.HasOne("StudentActivities.src.Models.Clubs", "Clubs")
                        .WithMany("Notifications")
                        .HasForeignKey("ClubId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Events", "Events")
                        .WithMany("Notifications")
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Clubs");

                    b.Navigation("Events");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Report", b =>
                {
                    b.HasOne("StudentActivities.src.Models.Admins", "Admins")
                        .WithMany("Reports")
                        .HasForeignKey("AdminId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Organizers", "Organizers")
                        .WithMany("Reports")
                        .HasForeignKey("OrganizerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Admins");

                    b.Navigation("Organizers");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Resgistrations", b =>
                {
                    b.HasOne("StudentActivities.src.Models.Clubs", "Clubs")
                        .WithOne("Resgistrations")
                        .HasForeignKey("StudentActivities.src.Models.Resgistrations", "ClubId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Events", "Events")
                        .WithOne("Resgistrations")
                        .HasForeignKey("StudentActivities.src.Models.Resgistrations", "EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Students", "Students")
                        .WithMany("Resgistrations")
                        .HasForeignKey("StudentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Clubs");

                    b.Navigation("Events");

                    b.Navigation("Students");
                });

            modelBuilder.Entity("StudentActivities.src.Models.TrainingScores", b =>
                {
                    b.HasOne("StudentActivities.src.Models.Semester", "Semester")
                        .WithMany("TrainingScores")
                        .HasForeignKey("SemesterId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Students", "Students")
                        .WithOne("TrainingScores")
                        .HasForeignKey("StudentActivities.src.Models.TrainingScores", "StudentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Semester");

                    b.Navigation("Students");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Admins", b =>
                {
                    b.HasOne("StudentActivities.src.Models.TrainingScores", "TrainingScores")
                        .WithMany()
                        .HasForeignKey("TrainingScoresId");

                    b.HasOne("StudentActivities.src.Models.Users", "Users")
                        .WithOne("Admins")
                        .HasForeignKey("StudentActivities.src.Models.Admins", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("TrainingScores");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Organizers", b =>
                {
                    b.HasOne("StudentActivities.src.Models.TrainingScores", "TrainingScores")
                        .WithMany()
                        .HasForeignKey("TrainingScoresId");

                    b.HasOne("StudentActivities.src.Models.Users", "Users")
                        .WithOne("Organizers")
                        .HasForeignKey("StudentActivities.src.Models.Organizers", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("TrainingScores");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Students", b =>
                {
                    b.HasOne("StudentActivities.src.Models.AcademicClasses", "AcademicClasses")
                        .WithMany("Students")
                        .HasForeignKey("AcademicClassId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("StudentActivities.src.Models.Users", "Users")
                        .WithOne("Students")
                        .HasForeignKey("StudentActivities.src.Models.Students", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AcademicClasses");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("StudentActivities.src.Models.AcademicClasses", b =>
                {
                    b.Navigation("Students");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Clubs", b =>
                {
                    b.Navigation("Notifications");

                    b.Navigation("Resgistrations");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Events", b =>
                {
                    b.Navigation("Notifications");

                    b.Navigation("Resgistrations");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Faculties", b =>
                {
                    b.Navigation("AcademicClasses");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Semester", b =>
                {
                    b.Navigation("TrainingScores");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Users", b =>
                {
                    b.Navigation("Admins");

                    b.Navigation("Organizers");

                    b.Navigation("Students");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Admins", b =>
                {
                    b.Navigation("Clubs");

                    b.Navigation("Events");

                    b.Navigation("Reports");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Organizers", b =>
                {
                    b.Navigation("Clubs");

                    b.Navigation("Events");

                    b.Navigation("Reports");
                });

            modelBuilder.Entity("StudentActivities.src.Models.Students", b =>
                {
                    b.Navigation("Clubs");

                    b.Navigation("Events");

                    b.Navigation("Resgistrations");

                    b.Navigation("TrainingScores");
                });
#pragma warning restore 612, 618
        }
    }
}
